...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `CODETTA_WORKSPACE` | `$HOME/codetta-workspace` | `.codetta` ファイルの相対 path 解決 |
...
codetta export-midi <PATH> --output <OUT.mid>
                            [--ppq <N>]
                            [--include-extensions text-meta|sidecar|none]
                            [--from <BEAT>] [--to <BEAT>]
```
...
| `--include-extensions` | codetta 拡張属性 (`master_gain` / fx / SF2 preset 詳細) の埋め込み方式 (デフォルト `text-meta`) |
...
- [ ] `export-midi` の `--include-extensions text-meta` で書き込む拡張属性のフォーマット (= JSON inline / key=value / カスタム) → Phase 3 (08-midi.md)
