...
> 音源は外付け SF2 (= schema 0.2)、 内蔵 synth は CDT-7 で完全削除済。
...
|---|---|
| `SCHEMA_VERSION` | `"0.2"` |
| `SUPPORTED_VERSIONS` | `["0.2"]` (0.1 は `io::load` で `UnknownVersion` reject、 `codetta migrate` 経由のみ) |
| `KNOWN_INSTRUMENT_TYPES` | `["soundfont"]` 1 種のみ |
| `add-track --instrument` 省略時 | `"soundfont"` |
| ドラム track での `pitch: "kick"` 等 (SF2 経路) | ✅ `synth::soundfont::DRUM_KEY_MIDI_MAP` 正本で render / validate 両方が正規化 (CDT-5) |
| `migrate` subcommand | ✅ 0.1 → 0.2 を LUT で自動変換 (CDT-6)。 `--manual` 不採用、 未掲載 type は preset 0 fallback + warning |
| 旧 internal `drum_kit` instrument | ❌ CDT-7 で削除 (migrate で SF2 preset 0 / bank 128 に変換される) |
| `export-midi` / `import-midi` | 未実装 (Phase 3 計画、 [08-midi.md](08-midi.md)) |

本 doc 内で「**Phase N 計画**」 と注記している subcommand は当該 Phase で実装する未来形。 注記の無い subcommand は **現状で動いている** 現役機能 (= 出力 JSON / フラグ名は現実装に揃えてある)。
...
| `migrate` | 1+ | ✓ | 旧バージョンスキーマからのアップグレード (0.1 → 0.2、 内蔵 synth → SF2 preset) |
...
`version` フィールドの値は `core::SCHEMA_VERSION` をそのまま出す (= 現状 `"0.2"`)。
...
| `--instrument <TYPE>` | 楽器 type。 schema 0.2 では `"soundfont"` 1 種のみ (= 既定)。 SF2 path / preset は `--params-json` で指定 |
...
| オプション | 説明 |
|---|---|
| `--type <TYPE>` | 楽器 type (必須)。 schema 0.2 では `soundfont` 一択 |
...
codetta set-instrument battle.codetta --track lead --type soundfont \
    --params-json '{"file":"GeneralUser-GS.sf2","preset":80,"bank":0}'
```
...
ドラムトラック (= SF2 bank 128) では `pitch` に要素名キー (`"kick"` / `"snare"` 等) を書ける。 内部で GM Drum Map (MIDI 番号) に正規化される (= CDT-5 で `synth::soundfont::DRUM_KEY_MIDI_MAP` を正本として実装済)。
...
利用可能な楽器一覧 + パラメータスキーマ。 schema 0.2 では **`"soundfont"` 1 entry のみ**。
...
`params` の各エントリは `note` / `unit` / `default` / `enum` / `range` / `required` の任意組合せ。 機械的に処理する場合は欠落フィールドを許容する形で読むこと。 `soundfont` entry には `drum_keys` フィールドが含まれ、 bank=128 (GM Drum) で使える要素名キー一覧 (kick / snare / hh_closed 等の 10 件) を返す。
...
{
  "ok": true,
  "file": "/abs/path/GeneralUser-GS.sf2",
  "soundfont": {
...
## `migrate` コマンド (実装済 / CDT-6)

旧バージョンスキーマファイルを現行版にアップグレードする。 当面 `0.1 → 0.2` のみサポート。 内蔵 synth (sin / saw / square / triangle / saw_pad / drum_kit + 用途別 alias) は LUT で `soundfont` preset/bank に置き換え、 旧 params (ADSR / pulse_width / kit 等) は破棄する。

```
codetta migrate <PATH> {--in-place | --output <OUT>} [--sf2 <FILE>]
```

| オプション | 説明 |
|---|---|
| `--in-place` | 同 path に上書き保存 (`--output` と排他、 どちらか必須) |
| `--output <OUT>` / `-o` | 別 path に出力 (`--in-place` と排他、 どちらか必須) |
| `--sf2 <FILE>` | migrate 後の SF2 ファイル (省略時 `GeneralUser-GS.sf2`。 Phase 4 で bundle 経路に確定予定) |

`--manual` は **採用しなかった**。 LLM (= MCP) 駆動を主な利用シナリオとした結果、 「対話的に各 track の preset を決める」 操作は MCP プロトコル上不自然 (= REPL 前提) で価値が低い。 LUT 未掲載 type は **fallback warning (`MIGRATE_UNKNOWN_INSTRUMENT`) + preset 0 (Acoustic Grand Piano)** に倒して migrate を完走させ、 必要なら後段で `codetta set-instrument` (= MCP の `set_instrument`) で個別調整するフローを取る。 これにより「対話的選択」 と機能等価。

**動作概要 (0.1 → 0.2、 LUT):**

| 0.1 の `instrument.type` | 0.2 への変換 |
|---|---|
| `sin` | `soundfont` / preset 38 / bank 0 (Synth Bass 1) — sin の dogfood 使用は sub bass 中心。 melody 用途は `set-instrument` で個別調整 |
| `saw` / `saw_lead` | `soundfont` / preset 81 / bank 0 (Saw Lead) |
| `square` / `square_bass` | `soundfont` / preset 80 / bank 0 (Square Lead) |
| `triangle` | `soundfont` / preset 73 / bank 0 (Flute) |
| `saw_pad` | `soundfont` / preset 88 / bank 0 (New Age Pad) |
| `drum_kit` (任意 `kit`) | `soundfont` / preset 0 / bank 128 (GM Standard Drum Kit)。 ノートの `pitch: "kick"` 等の要素名キーは migrate では変換しない (= 別途 `set-notes` で MIDI 番号 36/38/... に書き直すか、 CDT-5 で予定の drum pitch helper を待つ) |
| 上記以外 (例: `frobnicator`) | `soundfont` / preset 0 / bank 0 + warning。 `kit` などの 0.1 params は全廃棄 |

既に `soundfont` 型の track は **no-op** (= LUT 適用も warning も出さず、 `tracks_migrated` カウントから除外)。

既に 0.2 の song を migrate しようとすると `MIGRATE_NOT_NEEDED` エラーを返し、 出力ファイルは書き込まない。

**stdout (JSON、 実装に対応):**
```json
{
  "ok": true,
  "input": "/abs/path/in.codetta",
  "output": "/abs/path/out.codetta",
  "from_version": "0.1",
  "to_version": "0.2",
  "tracks_migrated": 3,
  "instrument_mapping": [
    { "track_id": "lead",  "from_kind": "saw_lead", "to_kind": "soundfont", "preset": 81, "bank": 0,   "fallback": false },
    { "track_id": "bass",  "from_kind": "sin",      "to_kind": "soundfont", "preset": 38, "bank": 0,   "fallback": false },
    { "track_id": "drums", "from_kind": "drum_kit", "to_kind": "soundfont", "preset": 0,  "bank": 128, "fallback": false }
  ]
}
```

LUT 未掲載 type が含まれていた場合は末尾に `warnings` 配列 (= `code` / `track_id` / `message` を持つ要素) が追加される。 出力ファイル自体の validate (preset 番号が SF2 に存在するか、 `pitch` が SF2 の音域に収まるか等) は migrate の責務外なので、 続けて `codetta validate` (MCP の `validate_song`) を呼ぶこと。

## Phase 3 計画コマンド (MIDI 連携)

詳細仕様は [08-midi.md](08-midi.md) に集約 (= CDT-2 ADR 確定済)。 本セクションは subcommand 形と必須オプションのみ載せる。
...
GM Program → SF2 preset マッピングの戦略は [08-midi.md](08-midi.md) で確定 (= melodic は同 program 番号、 SF2 に preset 無し時は preset 0 fallback + warning)。
...
[INFO] Loading SF2: GeneralUser-GS.sf2
[RENDER] ████████████████████ 100% (0.31s)
...
| `INVALID_JSON` | 1 | |
| `INVALID_SCHEMA` | 1 | |
| `UNKNOWN_VERSION` | 1 | `io::load` が 0.2 以外を弾く (= 0.1 ファイルは事前に `codetta migrate` で 0.2 化が必要) |
| `TRACK_NOT_FOUND` | 1 | |
...
codetta add-track battle.codetta --id lead --instrument soundfont \
    --params-json '{"file":"GeneralUser-GS.sf2","preset":81,"bank":0}'
...
codetta add-track battle.codetta --id bass --instrument soundfont \
    --params-json '{"file":"GeneralUser-GS.sf2","preset":38,"bank":0}'

# 5. ドラムトラック (GM Drum Kit = preset 0 / bank 128)
codetta add-track battle.codetta --id drums --instrument soundfont \
    --params-json '{"file":"GeneralUser-GS.sf2","preset":0,"bank":128}'
...
- [x] `migrate` の LUT で内蔵 synth → SF2 preset を完全自動マッピングするか、 dogfood 上 OK なものに絞って残りは `--manual` 必須にするか → **CDT-6 で全 type を LUT 自動変換 + 未掲載 type は preset 0 fallback + warning に確定**。 `--manual` 採用せず、 後段で `set-instrument` 個別調整するフロー
- [x] `export-midi` の channel 割当 (= 同時に鳴らす track が 16 を超えた場合の戦略) → [08-midi.md](08-midi.md) で確定 (= `TRACK_LIMIT_EXCEEDED` エラー、 自動 split / merge しない)
- [x] `export-midi` の `--extensions text-meta` で書き込む拡張属性のフォーマット → [08-midi.md](08-midi.md) で確定 (= `MTrk` 0 先頭に JSON inline)
- [x] `import-midi` の SF2 preset 自動マッピング失敗時の振る舞い → [08-midi.md](08-midi.md) で確定 (= preset 0 fallback + warning)
