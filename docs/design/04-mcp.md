...
詳細仕様は (Phase 3 で起こす) `docs/design/08-midi.md` に集約予定。 ここでは tool 名 / input / output を予約しておく (= CLI subcommand `export-midi` / `import-midi` と pair で expose、 [03-cli.md](03-cli.md) 「Phase 3 計画コマンド」 と整合)。
...
`extensions` = codetta 拡張属性 (`master_gain` / fx / SF2 preset 詳細) の埋め込み方式 (= Phase 3 で 08-midi.md 詰める段で default 含め最終確定)。
...
GM Program → SF2 preset 自動マッピング (= preset 番号 100% 一致しない SF2 への扱い) の戦略は Phase 3 で 08-midi.md に集約。
...
- [ ] `export_midi` の `extensions` default は `text-meta` / `sidecar` / `none` のどれにするか → Phase 3 (08-midi.md)
- [ ] `import_midi` の `sf2` 省略時挙動 (= bundle SF2 fallback、 Phase 4 配布整備依存) を Phase 3 単独リリース時にどう扱うか → Phase 3 着手時に確定
- [ ] `import_midi` で SF2 preset 自動マッピング失敗時の振る舞い (preset 0 fallback / channel skip / エラー) → Phase 3 (08-midi.md)
