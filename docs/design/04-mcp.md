...
Claude:
  → list_soundfont_presets(file="GeneralUser-GS.sf2") で preset 確認
  → create_song(path="ddc-battle.codetta", bpm=140, key="Am", name="Cyber Battle Loop", master_gain=2.0)
  → add_track(id="lead", instrument="soundfont", params={file:"GeneralUser-GS.sf2", preset:81, bank:0})  # Saw Lead
  → set_fx(track_id="lead", fx=[{type:"delay",time:"1/8",mix:0.3},{type:"reverb",size:0.4,mix:0.2}])
  → set_notes(track_id="lead", notes=[ ...8 ノート ...])
  → add_track(id="bass", instrument="soundfont", params={file:"GeneralUser-GS.sf2", preset:38, bank:0}, volume=0.9)  # Synth Bass 1
  → set_notes(track_id="bass", notes=[ ...4 ノート ...])
  → add_track(id="drums", instrument="soundfont", params={file:"GeneralUser-GS.sf2", preset:0, bank:128})  # GM Standard Drum Kit
...
Claude:
  ...
  → set_instrument(track_id="lead", type="soundfont", params={file:"GeneralUser-GS.sf2", preset:80, bank:0})  # Square Lead に差し替えてエッジを出す
