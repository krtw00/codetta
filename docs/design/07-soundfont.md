...
> Codetta の音源は外付け SoundFont (SF2) で統一する。 内蔵 synth は持たない (= schema 0.2 化で完全撤去予定)。
...
| 項目 | 現実装 | Phase 2 完了後 (= 本 doc spec) |
|---|---|---|
| `SCHEMA_VERSION` | `"0.1"` | `"0.2"` |
| 内蔵 synth (sin / saw / saw_lead / square / square_bass / triangle / saw_pad / drum_kit) | 残存 | **完全撤去** |
...
`render::render_to_buffer` 内の dispatch は `kind == "soundfont"` で分岐 (Phase 1+ では drum / melodic と並列、 Phase 2 では soundfont 1 種に縮退)。 SF2 load 失敗時は track をスキップして `eprintln` 警告 (= validate で報告される責務)。
...
同一 SF2 ファイルを複数 track で参照する場合、 ファイル load + parse を track ごとに重複させない。 `render::render_to_buffer` で SF2 path → `Arc<SoundFont>` の HashMap を持ち、 同じ path が出てきたら共有 (= load 重複回避)。 これは Phase 1+ で実装済。
